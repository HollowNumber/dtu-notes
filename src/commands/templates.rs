...
pub fn create_custom_template(course_id: &str, title: &str, template_type: &str, sections: Option<&str>) -> Result<()> {
    ...
    OutputManager::print_status(
        Status::Loading,
        &format!("Creating custom {} template for {}", template_type, course_id.yellow())
    );
    ...
    let output_dir = Path::new(&config.paths.notes_dir).join(course_id).join("custom-templates");
    ...
    OutputManager::print_status(
        Status::Success,
        &format!("Custom template created: {}", file_path.display().to_string().bright_white())
    );
    ...
    println!("  Course: {} - {}", 
             course_id.yellow(), 
             config.courses.get(course_id).unwrap_or(&"Unknown Course".to_string()).dimmed());
    ...
    println!();
    OutputManager::print_command_examples(&[
        (&format!("noter compile {}", file_path.display()), "Compile to PDF"),
        (&format!("noter watch {}", file_path.display()), "Watch and auto-compile"),
    ]);
